...
class ExamplePlugin(base_plugin.TBPlugin):
  ...
  def __init__(self, context):
    """Instantiates ExamplePlugin via TensorBoard core.
  ...
  def _serve_runs(self, request):
    return http_util.Respond(request, json.dumps(self._multiplexer.Runs()), 'application/json')
  ...
  def _serve_file(self, request):
    ...
    prefix = "/data/plugin/example_basic/"
  ...
  def scalars_impl(self, tag, run, experiment):
    ...
    if self._data_provider:
      ...
      all_scalars = self._data_provider.read_scalars(
          experiment_id=experiment,
          plugin_name=scalar_plugin_name,
      ...
      ''', dict(exp=experiment, run=run, tag=tag, plugin=scalar_plugin_name))
  ...
  def scalars_route(self, request):
    ...
    print("!!!!on the scalars route")
    tag = request.args.get('tag')
    run = request.args.get('run')
    print(tag, run)
    experiment = plugin_util.experiment_id(request.environ)
    (body, mime_type) = self.scalars_impl(tag, run, experiment)
